			...
			>
				<div className='flex laptop:flex-row flex-col laptop:w-1220 w-full flex-wrap laptop:justify-between text-center text-2xl font-bold text-cardColor'>
					{news.map(el => {
						return (
							<div className='flex flex-col laptop:w-388 w-full laptop:mt-6 laptop:mb-0 mb-6 px-8'>
								<Link href={`/newsPage/${el.id}`} className={'hover:underline text-lg'}>
								<Image
									className={'rounded-xl'}
									src={`https://api.toomoyun.kg${el.img}`}
									width={388}
									height={388}
									alt=''
								/>
					...
					{ads.map(el => {
						return (
							<div className='flex flex-col laptop:w-388 w-full laptop:mt-6 laptop:mb-0 mb-6 px-8'>
								<Link href={`/garNews/${el.id}`} className={'hover:underline text-lg'}>
								<Image
									src={`https://api.toomoyun.kg${el.img}`}
									width={388}
									height={388}
					...
					{gallery.map(el => {
						return (
							<div className='laptop:w-364 w-full flex justify-center laptop:mb-0 mb-10 mt-5'>
								<Image
									className='w-364 h-364 rounded-xl'
									src={`https://api.toomoyun.kg${el.photo}`}
									alt=''
									width={388}
									height={388}
								/>
