...
const Gallery: FC<IGalleryData> = ({gallery}) => {
	...
	return <div className='text-center laptop:my-24 my-5 flex flex-col items-center'>
		<h1 className='text-xl font-bold mb-12 text-red-600'>Фотогалерея</h1>
		<div className='flex laptop:flex-wrap laptop:flex-row flex-col laptop:w-1140 w-full laptop:justify-between justify-start laptop:items-start items-center laptop:h-670 h-auto laptop:mb-16 mb-5'>
			...
			<Image className='mt-8' src={gallery.results[2] === null ? '' : gallery.results[2].photo} alt='' width={360} height={320} />
			<Image className='laptop:w-750 w-360 h-320 mt-8' src={gallery.results[3] === null ? '' : gallery.results[3].photo} alt='' width={360} height={320} />
		</div>
		<Button href='/photoGallery' className='w-56 text-4xl bg-blue-600 px-2 py-3 rounded-xl'>Фотографияларга өтүү</Button>
	</div>
}
